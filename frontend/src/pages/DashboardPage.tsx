...
function SkeletonCard() {
  return <div className="rounded-2xl bg-slate-100 animate-pulse h-36" />;
}
    ...
    <div className="min-h-screen bg-[#f7f7f5]">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900 px-6 py-10 sm:px-10 sm:py-14">
        ...
        <div className="relative mx-auto max-w-7xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <p className="text-xs font-semibold tracking-[0.2em] uppercase text-emerald-400 mb-2">
              ESG · Analyst Portal
            </p>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Overview of ingested activity data awaiting analyst sign-off.
            </p>
          </div>
          <Link
            to="/upload"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
          {isLoading
            ...
            : cards.map((card) => (
                ...
                >
                  <div
                    className={`inline-flex items-center justify-center rounded-lg ${card.accent}/30 p-2 ${card.text} mb-3`}
                  >
                    {card.icon}
                  </div>
                  <p
                    className={`text-[11px] font-semibold uppercase tracking-widest ${card.text} opacity-70`}
                  >
                    {card.label}
                  </p>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Latest ingestion batches from all sources
              </p>
            </div>
            <Link
              to="/upload"
                ...
                {isLoading ? (
                  ...
                  data?.recent_batches.map((batch) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`font-semibold ${batch.error_count > 0 ? "text-red-500" : "text-slate-400"}`}
                        >
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`font-semibold ${batch.flagged_count > 0 ? "text-orange-500" : "text-slate-400"}`}
                        >
