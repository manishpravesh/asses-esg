    ...
    <tr className="border-t border-slate-100">
      {[20, 14, 16, 12, 18, 14, 12, 10].map((w, i) => (
        <td key={i} className="px-4 py-4">
          <div
            className={`h-3 bg-slate-100 rounded-full animate-pulse w-${w}`}
          />
        </td>
      ))}
    </tr>
...
export default function ReviewPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      (
        await api.get<{ results: Activity[] }>("/activities/", {
          params: tab ? { review_status: tab } : {},
        })
      ).data,
  });
  ...
  const bulk = useMutation({
    mutationFn: (ids: string[]) =>
      api.post("/activities/bulk-approve/", { activity_ids: ids }),
    onSuccess: () => {
        ...
        <div className="relative mx-auto max-w-7xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Approve normalized activity rows before they are locked for audit.
            </p>
          </div>
                    ...
                    <td colSpan={8} className="px-5 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs text-slate-400">
                          {tab
                            ? `No ${TABS.find((t) => t.key === tab)?.label.toLowerCase()} items at the moment.`
                            : "Upload data to get started."}
                        </p>
                        {!tab && (
                          <Link
                            to="/upload"
                            className="mt-1 inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 hover:text-emerald-700"
                          >
                        ...
                        >
                          {r.flag_reasons?.length ? (
                            <>
                              ...
                              {r.flag_reasons.length} flag
                              {r.flag_reasons.length > 1 ? "s" : ""}
                            </>
                          ) : (
                            <>
                              View
